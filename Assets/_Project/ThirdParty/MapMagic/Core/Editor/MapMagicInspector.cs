						...
						using (Cell.LineStd) 
						{
							...
							using (Cell.LineStd) 
							{
								using (Cell.RowRel(1-Cell.current.fieldWidth))
									using (Cell.LineStd) //to keep on-top when list enabled, not in center
										Draw.Label("Around Objects");

								using (Cell.RowRel(Cell.current.fieldWidth))
								{
									using (Cell.RowPx(20))
										using (Cell.LineStd)
											Draw.Toggle(ref mapMagic.tiles.genAroundTfms);

									using (Cell.Row)
									{
										Cell.current.disabled = !mapMagic.tiles.genAroundTfms;
										LayersEditor.DrawLayers(ref mapMagic.tiles.genAroundTfmsList, onDraw: n => 
											{ 
												using (Cell.LineStd)
												{
													using (Cell.RowPx(20)) Draw.Icon(UI.current.textures.GetTexture("DPUI/Icons/Layer"));
													using (Cell.Row) Draw.Field(ref mapMagic.tiles.genAroundTfmsList[n], allowSceneObject:true);
												}
											} );
									}
								}
							...
							using (Cell.LineStd) 
							{
								using (Cell.RowRel(1-Cell.current.fieldWidth))
									using (Cell.LineStd) //to make it at top line
										Draw.Label("Around Coordinates");

								using (Cell.RowRel(Cell.current.fieldWidth))
								{
									using (Cell.RowPx(20))
										using (Cell.LineStd) 
											Draw.Toggle(ref mapMagic.tiles.genAroundCoordinates);

									using (Cell.Row)
									{
										Cell.current.disabled = !mapMagic.tiles.genAroundCoordinates;
										LayersEditor.DrawLayers(ref mapMagic.tiles.genCoordinates, onDraw: n => 
											{ 
												using (Cell.RowPx(20)) Draw.Icon(UI.current.textures.GetTexture("DPUI/Icons/Layer"));
												using (Cell.Row) Draw.Field(ref mapMagic.tiles.genCoordinates[n]);
											} );
									}
								}
							}
