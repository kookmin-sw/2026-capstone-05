...
public class PlayerConditionUI : MonoBehaviour
{
    ...
    private void Update()
    {
        if (player == null && !TryResolvePlayerCondition())
        {
            return;
        }
    ...
    private bool TryResolvePlayerCondition()
    {
        if (player != null)
        {
            return true;
        ...
        {
            if (controller != null && controller.IsLocalPlayer && controller.Condition != null)
            {
                player = controller.Condition;
                break;
            }
        }
        ...
        if (player == null)
        {
            Debug.LogWarning("[PlayerConditionUI] PlayerCondition을 찾지 못했습니다. UI 업데이트를 건너뜁니다.");
            return false;
        }
