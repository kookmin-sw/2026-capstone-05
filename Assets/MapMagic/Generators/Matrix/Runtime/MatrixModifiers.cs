	...
	*/
	...
	public class Distort219 : Generator, IInlet<MatrixWorld>, IMultiInlet, IOutlet<MatrixWorld> 
	{
		...
		public override void Generate (TileData data, StopToken stop)
		{
			...
			if (xMask != null || zMask != null)
				for (int x=0; x<src.rect.size.x; x++)
					for (int z=0; z<src.rect.size.z; z++)
					{
						int i = z*src.rect.size.x + x;

						float xOffset = 0;
						if (xMask != null)
						{
							xOffset = (xMask.arr[i] - midpoint) * worldIntensity;
							if (x+xOffset < 0)
								xOffset = -x;
						...
						if (zMask != null)
						{
							zOffset = (zMask.arr[i] - midpoint) * worldIntensity;
							if (z+zOffset < 0)
								zOffset = -z;
						...
						float val = src.GetInterpolated(x+dst.rect.offset.x + xOffset, z+dst.rect.offset.z + zOffset);
						dst.arr[i] = val;
					}
	...
	public class Selector200 : Generator, IInlet<MatrixWorld>, IOutlet<MatrixWorld>
	{
		...
		{
			tFrom = (this.from.x + this.from.y)/2;
			tTo = (this.to.x + this.to.y)/2;
			tTransition = (this.from.y - this.from.x);
		}

		public void SwitchRangeTransitionToMinmax ()
		{
			from.x = tFrom-tTransition/2;
			from.y = tFrom+tTransition/2;
			to.x = tTo-tTransition/2;
			to.y = tTo+tTransition/2;
		}
